...
NUM_FRAMES = (
    1348  # Number of frames ~ TIME * FRAME_RATE - 1 (for long streams we subtract >1 )
)
...
class SignalProcessor:
    ...
    def process_signal_pipeline(self, raw_signal):
        # MTI (highpass)
        heart = lfilter(
            self.hp_b_heart, self.hp_a_heart, raw_signal - np.mean(raw_signal)
        )
        breath = lfilter(
            self.hp_b_breath, self.hp_a_breath, raw_signal - np.mean(raw_signal)
        )
...
def breathing_peak_detect(breath_filtered, fs_breath, window_sec=20):
    ...
    for i in range(num_windows):
        # Extract Window
        current_window = breath_filtered[
        ...
        peaks, _ = find_peaks(
            current_window, distance=int(2 * fs_breath), prominence=0.15
        )
        # Mumber of peaks
