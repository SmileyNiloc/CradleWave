...
class SignalProcessor:
    ...
    def __init__(self, sample_rate):
        ...
        self.bp_b_heart, self.bp_a_heart = butter(6, [0.8 / self.nyquist, 2.5 / self.nyquist], "band")

        # Breathing filters
        # MTI Highpass filter
        self.hp_b_breath, self.hp_a_breath = butter(3, 0.07 / self.nyquist, "highpass")
        # Bandpass Filter: 4-24bpm range
        self.bp_b_breath, self.bp_a_breath = butter(3, [0.07 / self.nyquist, 0.4 / self.nyquist], "band")
    ...
    def moving_target_indicator_heart(self, signal_data):
        ...
        return lfilter(self.hp_b_heart, self.hp_a_heart, signal_data - np.mean(signal_data))

    def moving_target_indicator_breath(self, signal_data):
        # MTI filter to remove static clutter
        # 3rd order butterworth high-pass filter
        return lfilter(self.hp_b_breath, self.hp_a_breath, signal_data - np.mean(signal_data))
    ...
    def heart_peak_detect(self, signal_data, frame_rate, window):
        """
        ...
        num_windows = (len(signal_data)+1) // samples_per_window
        ...
        for i in range(num_windows):
            ...
            peaks, _ = find_peaks(chunk, distance=int(0.2 * frame_rate), prominence = 0.15)
    ...
    def breathing_peak_detect(self, signal_data, frame_rate, window):
        """
        ...
        for i in range(num_windows):
            # Extract Window
            current_window = signal_data[i * samples_per_window : (i + 1) * samples_per_window]
            # Peaks Detection
            ...
            peaks, _ = find_peaks(current_window, distance=int(2 * frame_rate), prominence = 0.15)
            # Mumber of peaks
    ...
    def process_signal_pipeline(self, raw_signal):
        ...
        filtered_heart = self.sliding_average_filter_heart(bp_filtered_heart, window_size=5)
        filtered_breath = self.sliding_average_filter_breath(bp_filtered_breath, window=3)

        # Step 4: Estimate heart rate -ATTENTION: BASED ON 20 SAMPLE PARTITION WITHIN SIGNAL_DATA, SEE ABOVE COMMENT
        bpm = self.heart_peak_detect(filtered_heart, self.sample_rate, window=20)
        breath = self.breathing_peak_detect(filtered_breath, self.sample_rate, window=20)
