...
if __name__ == "__main__":
    ...
    for i in range(samples):
        ...
        publish_future, packet_id = mqtt_connection.publish(
            ...
            qos=mqtt.QoS.AT_LEAST_ONCE,  # Equivalent to QoS 1
        )
        # print(
        #     f"Publishing message {i+1}: {json_payload[:100]}... to topic '{TOPIC}'..."
        # )
        publish_future.result()
    # Wait for the AWS server to acknowledge the message was received
    ...
    print(
        f"Average bytes per second: {(len(json_payload)*samples)/total_time:.2f} bytes/sec"
    )
